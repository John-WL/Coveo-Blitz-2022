...
public class BiTotemSolver implements CoveoSolver {
    @Override
    public List<TotemAnswer> solve(List<TotemQuestion> totemsToPlace) {
        if(totemsToPlace.size() != 2) {
            throw new RuntimeException("BiTotemSolver object needs a problem with exactly two totems");
        }
        ...
        for(int i = 0; i < snowFlakes.size(); i++) {
            snowFlakes.get(i).moveBy(new CoordinatePair(i*4, 0));
        }
